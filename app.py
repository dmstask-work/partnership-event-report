...
app.layout = dbc.Container([
    ...
    dbc.Tabs([
        ...
        #  TAB 1 — Hadir Report
        # ════════════════════════════════════════════════════════════════════
        dbc.Tab(label="📋 Hadir Report", tab_id="tab-hdr",
                label_style={"fontWeight": "600"},
                active_label_style={"color": "#2d7dd6"},
                children=[
    ...
    dbc.Row(dbc.Col(
        ...
        #  TAB 2 — WP Report
        # ════════════════════════════════════════════════════════════════════
        dbc.Tab(label="📱 WP Report", tab_id="tab-wp",
                label_style={"fontWeight": "600"},
                active_label_style={"color": "#2d7dd6"},
                children=[
            ...
            dbc.Row([
                dbc.Col([
                    html.Label("Sesi / Nama Event", className="fw-semibold small text-muted mb-1"),
                    dcc.Dropdown(
                        id="wp-dd-sesi",
                        options=[{"label": "✦ Semua Sesi", "value": "ALL"}] + (
                            [{"label": s, "value": s}
                             for s in sorted([x for x in df_wp_raw["Nama Event"].unique()
                ...
                dbc.Col([
                    html.Label("Provinsi", className="fw-semibold small text-muted mb-1"),
                    dcc.Dropdown(
                        id="wp-dd-provinsi",
                        options=[{"label": "✦ Semua Provinsi", "value": "ALL"}] + (
                            [{"label": p, "value": p}
                             for p in sorted([x for x in df_wp_raw["Provinsi"].dropna().unique()
                                              if isinstance(x, str) and x not in ("-", "nan")])]
                            if not df_wp_raw.empty else []
                        ),
                        value="ALL", clearable=False,
                ...
                dbc.Col([
                    html.Label("Country", className="fw-semibold small text-muted mb-1"),
                    dcc.Dropdown(
                        id="wp-dd-country",
                        options=[{"label": "✦ Semua Negara", "value": "ALL"}] + (
                            [{"label": c, "value": c}
                             for c in sorted([x for x in df_wp_raw["Country"].dropna().unique()
                                              if isinstance(x, str) and x not in ("-", "nan")])]
                            if not df_wp_raw.empty else []
                        ),
                        value="ALL", clearable=False,
            ...
            dbc.Row(dbc.Col(
                html.Div([
                    html.H6("Rekap Peserta WP", className="fw-bold mb-3 text-secondary"),
       ...
       style={"borderBottom": "2px solid #ADD3FA"},
       className="mb-0"),
...
def update_wp(sesi, provinsi, country):
    ...
    # ── Provinsi ─────────────────────────────────────────────────────────
    prov_s  = df[df["Provinsi"].apply(lambda x: isinstance(x, str) and x not in ("-", "nan"))]["Provinsi"]
    ...
    # ── Country ──────────────────────────────────────────────────────────
    ctry_s  = df[df["Country"].apply(lambda x: isinstance(x, str) and x not in ("-", "nan"))]["Country"]
    ...
    # ── District ─────────────────────────────────────────────────────────
    dist_s  = df[df["District"].apply(lambda x: isinstance(x, str) and x not in ("-", "nan"))]["District"]
    dist_df = dist_s.value_counts().head(15).reset_index()
    dist_df.columns = ["District", "Entri"]
    fig_dist = px.bar(
        dist_df, x="Entri", y="District", orientation="h",
        title="Top 15 Kota / Distrik",
        color="Entri", color_continuous_scale=BLUE_SCALE, text="Entri",
    )
    fig_dist.update_traces(textposition="outside", marker_line_width=0)
    fig_dist.update_layout(**CHART_LAYOUT, coloraxis_showscale=False,
                           yaxis=dict(categoryorder="total ascending", title=""),
                           xaxis=dict(title=""))
    ...
    return kpis, fig_sesi, fig_gen, fig_prov, fig_ctry, fig_dist, wp_table
